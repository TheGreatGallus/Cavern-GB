...
extern UINT8 scroll_top_movement_limit = 50;
extern UINT8 scroll_bottom_movement_limit = 60;
...
void ScrollSetTiles(UINT8 first_tile, const struct TilesInfo* tiles) {
	...
	//set_bkg_data(45, 1, tiles->data->data+(16*43));
...
UINT8 frame = 0;
struct MapInfo* curr_map_data;
void UpdateWaterfallTiles() {

	struct TilesInfo* tiles_info;

	//Init Tiles
	PUSH_BANK(curr_map_data->bank)
		tiles_info = curr_map_data->data->tiles;
	POP_BANK;

	PUSH_BANK(tiles_info->bank);
	if (frame == 120)
		frame = 0;
	// 47 = Waterfall
	set_bkg_data(47, 1, tiles_info->data->data + (16 * (43 + ((frame / 15) % 4))));
	frame++;	
	POP_BANK;
}
...
void UpdateMushroomGlowTiles() {
	...
	if (frame == 120)
		frame = 0;
	// 100 = Right Mushroom
	set_bkg_data(100, 1, tiles_info->data->data + (16 * (92 + ((frame / 15) % 8))));
	// 91 = Left Mushroom
	set_bkg_data(91, 1, tiles_info->data->data + (16 * (83 + ((frame / 15) % 8))));
	frame++;
	POP_BANK;
}
...
void UpdateTorchTiles() {
	...
	if (frame == 120)
		frame = 0;
	//// 115 = TL Torch
	set_bkg_data(115, 1, tiles_info->data->data + (16 * (113 + ((frame / 30) % 2))));
	//// 118 = TR Torch
	set_bkg_data(118, 1, tiles_info->data->data + (16 * (116 + ((frame / 30) % 2))));
	//// 118 = TR Torch
	set_bkg_data(121, 1, tiles_info->data->data + (16 * (119 + ((frame / 30) % 2))));
	//// 118 = TR Torch
	set_bkg_data(124, 1, tiles_info->data->data + (16 * (122 + ((frame / 30) % 2))));
	frame++;
	POP_BANK;
}
...
void UnlockDoors() {
	...
	PUSH_BANK(tiles_info->bank);
	// 1 = Lock
	set_bkg_data(1, 1, tiles_info->data->data);
	scroll_collisions[1] = 0u;
	frame++;
...
//	PUSH_BANK(map->bank);
//	set_win_tiles(x, y, w, h, map->data->data);
//
//#ifdef CGB
//	if (map->data->attributes) {
//		VBK_REG = 1;
...
void RefreshScroll() {
	...
	if(scroll_target) {
		...
		MoveScroll(scroll_target->x + 16 - (SCREENWIDTH >> 1), ny);
	}
}
